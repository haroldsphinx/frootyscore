...
function sizeHandler() {
    ...
    if ($('#canvas')) {
        ...
        k = d / 2 - c / 2,
        h = CANVAS_WIDTH / c;
        if (k * h < - EDGEBOARD_X || f * h < - EDGEBOARD_Y) b = Math.min(a / (CANVAS_HEIGHT - 2 * EDGEBOARD_Y), d / (CANVAS_WIDTH - 2 * EDGEBOARD_X)),
        c = CANVAS_WIDTH * b,
        b *= CANVAS_HEIGHT,
        f = (a - b) / 2,
        k = (d - c) / 2,
...
function getBounds(a, d) {
    ...
    if (a instanceof createjs.Container) {
        ...
        k,
        h;
        for (h = 0; h < f; h++) k = getBounds(c[h], 1),
        k.x < b.x && (b.x = k.x),
        k.y < b.y && (b.y = k.y),
        ...
        var e,
        g;
        a instanceof createjs.Bitmap ? (f = a.sourceRect || a.image, h = f.width * d, e = f.height * d)  : a instanceof createjs.Sprite ? a.spriteSheet._frames && a.spriteSheet._frames[a.currentFrame] && a.spriteSheet._frames[a.currentFrame].image ? (f = a.spriteSheet.getFrame(a.currentFrame), h = f.rect.width, e = f.rect.height, c = f.regX, g = f.regY)  : (b.x = a.x || 0, b.y = a.y || 0)  : (b.x = a.x || 0, b.y = a.y || 0);
        c = c || 0;
        h = h || 0;
        g = g || 0;
        e = e || 0;
...
TEXT_SCORE = 'SCORE: ';
TEXT_PAUSE = 'PAUSE';
HELP_TEXT_DESKTOP = 'USE THE ARROWS TO POSITION THE BALL THEN PRESS SPACEBAR TO SHOOT!!';
HELP_TEXT_MOBILE = 'USE THE ARROWS TO POSITION THE BALL THEN TAP ON THE SCREEN TO SHOOT!!';
HELP_TEXT = 'TRY TO BE QUICK! IT INCREASES YOUR SCORE, LOG ON TO FACEBOOK TO SUBMIT YOUR SCORES AND STAND A CHANCE OF WINNING FANTASTIC PRIZES FROM ETISALAT.';
...
function CMain(a) {
    ...
    this._initSounds = function () {
        createjs.Sound.initializeDefaultPlugins() && (0 < navigator.userAgent.indexOf('Opera') || 0 < navigator.userAgent.indexOf('OPR') ? (createjs.Sound.alternateExtensions = [
            'mp3'
        ], createjs.Sound.addEventListener('fileload', createjs.proxy(this.soundLoaded, this)), createjs.Sound.registerSound('./sounds/soundtrack.ogg', 'soundtrack'), createjs.Sound.registerSound('./sounds/press_but.ogg', 'click'), createjs.Sound.registerSound('./sounds/applause.ogg', 'applause'), createjs.Sound.registerSound('./sounds/crowd.ogg', 'crowd'), createjs.Sound.registerSound('./sounds/goal.ogg', 'goal'), createjs.Sound.registerSound('./sounds/keeper_save.ogg', 'keeper_save'), createjs.Sound.registerSound('./sounds/kick.ogg', 'kick'), createjs.Sound.registerSound('./sounds/miss_goal.ogg', 'miss_goal'), createjs.Sound.registerSound('./sounds/select_team.ogg', 'select_team'), createjs.Sound.registerSound('./sounds/game_over.ogg', 'game_over'), createjs.Sound.registerSound('./sounds/stop_indicator.ogg', 'stop_indicator'))  : (createjs.Sound.alternateExtensions = [
            'ogg'
        ], createjs.Sound.addEventListener('fileload', createjs.proxy(this.soundLoaded, this)), createjs.Sound.registerSound('./sounds/soundtrack.mp3', 'soundtrack'), createjs.Sound.registerSound('./sounds/press_but.mp3', 'click'), createjs.Sound.registerSound('./sounds/applause.mp3', 'applause'), createjs.Sound.registerSound('./sounds/crowd.mp3', 'crowd'), createjs.Sound.registerSound('./sounds/goal.mp3', 'goal'), createjs.Sound.registerSound('./sounds/keeper_save.mp3', 'keeper_save'), createjs.Sound.registerSound('./sounds/kick.mp3', 'kick'), createjs.Sound.registerSound('./sounds/miss_goal.mp3', 'miss_goal'), createjs.Sound.registerSound('./sounds/select_team.mp3', 'select_team'), createjs.Sound.registerSound('./sounds/game_over.mp3', 'game_over'), createjs.Sound.registerSound('./sounds/stop_indicator.mp3', 'stop_indicator')), c += 11)
    };
    this._loadImages = function () {
        ...
        s_oSpriteLibrary.addSprite('bg_select_team', './sprites/bg_select_team.png');
        ...
        s_oSpriteLibrary.addSprite('messi1_idle', './sprites/players/messi1_idle.png');
        s_oSpriteLibrary.addSprite('messi1_idle2', './sprites/players/messi2_idle.png');
        s_oSpriteLibrary.addSprite('neymar1_idle', './sprites/players/neymar1_idle.png');
        s_oSpriteLibrary.addSprite('neymar1_idle2', './sprites/players/neymar2_idle.png');
        s_oSpriteLibrary.addSprite('suarez1_idle', './sprites/players/suarez1_idle.png');
        s_oSpriteLibrary.addSprite('suarez1_idle2', './sprites/players/suarez2_idle.png');
        s_oSpriteLibrary.addSprite('messi1_shot', './sprites/players/messi1_shot.png');
        s_oSpriteLibrary.addSprite('neymar1_shot', './sprites/players/neymar1_shot.png');
        s_oSpriteLibrary.addSprite('suarez1_shot', './sprites/players/suarez1_shot.png');
...
function CSelectTeam() {
    ...
    this._init = function () {
        ...
        e = new createjs.Text('MESSI', ' 25px ' + TEXT, '#080863');
    ...
    this._onModeNext = function () {
        2 !== k && (f += 1, this._onModeToggle(f, 0))
    };
    this._onModePrevious = function () {
        0 !== f && (--f, this._onModeToggle(f, 0))
    };
    ...
    this._onButNextRelease = function () {
        ...
        switch (k) {
            case 0:
                ...
                break;
            case 1:
                a = 0 === h ? 'neymar1' : 'neymar2';
                break;
            case 2:
                a = 0 === h ? 'suarez1' : 'suarez2';
                break;
            case 3:
                a = 'england';
                break;
...
function CGame(a, d) {
    ...
    T;
    this._init = function () {
        0 !== q || !1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || (s_oSoundtrack.volume = 0, s_oCrowd = createjs.Sound.play('crowd', {
            loop: - 1
        }));
        c = 1000;
        l = g = - 1;
        ...
        s_oStage.addChild(a);
        W = new CCrowd;
        0 === n ? this._initLevel()  : (x = new CInterface(m), this.createViewThings());
    ...
    this.createViewThings = function () {
        ...
        s_oStage.addChild(T);
        if (0 < B.num) for (var a = 0; a < B.num; a++) K[a] = new CWall(B.x, B.y, T, a),
        K[a].showIdle(a);
    ...
    this.showMessage = function (a) {
        ...
        var e = this;
        !0 === a ? (y.fadeOut(), a = createBitmap(s_oSpriteLibrary.getSprite('missed_text')), a.scaleX = 0, a.scaleY = 0, a.alpha = 0, a.x = CANVAS_WIDTH / 2, a.y = CANVAS_HEIGHT / 2, a.regX = 206.5, a.regY = 37, s_oStage.addChild(a), createjs.Tween.get(a).to({
            alpha: 1,
            scaleX: 1,
            scaleY: 1
        }, 500).wait(800).call(function () {
            e.controlIfCanContinue()
        }))  : !0 === t ? (a = createBitmap(s_oSpriteLibrary.getSprite('missed_text')), a.scaleX = 0, a.scaleY = 0, a.alpha = 0, a.x = CANVAS_WIDTH / 2, a.y = CANVAS_HEIGHT / 2, a.regX = 206.5, a.regY = 37, s_oStage.addChild(a), createjs.Tween.get(a).to({
            alpha: 1,
            scaleX: 1,
            scaleY: 1
        }, 500).wait(800).call(function () {
            e.controlIfCanContinue()
        }))  : l === OUT ? (a = createBitmap(s_oSpriteLibrary.getSprite('out_text')), a.scaleX = 0, a.scaleY = 0, a.alpha = 0, a.x = CANVAS_WIDTH / 2, a.y = CANVAS_HEIGHT / 2, a.regX = 130.5, a.regY = 35, s_oStage.addChild(a), !1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || createjs.Sound.play('miss_goal'), y.fadeOut(), createjs.Tween.get(a).to({
            alpha: 1,
            scaleX: 1,
            scaleY: 1
        }, 500).wait(800).call(function () {
    ...
    this.controlIfCanContinue = function () {
        f >= k && 1 >= h ? (m++, n = 0, m === NUM_LEVEL ? (c = 0, L = CEndPanel(s_oSpriteLibrary.getSprite('bg_win'), s_oSpriteLibrary.getSprite('you_win')), L.win(b))  : (this.unload(), this._init()))  : 1 >= h && this.gameOver();
        1 < h && (h--, n++, this.unload(), this._init())
    };
    ...
    this.onExit = function () {
        ...
        this.unload();
        s_oMain.gotoMenu();
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) s_oCrowd.volume = 0,
    ...
    this.update = function () {
        if (u) {
            if (v && (e = C.getFrame(), 4 === e && !z)) {
                if (Math.floor(100 * Math.random()) <= g && 0 < l && 0 < g) H = l,
                l !== OUT && (t = !0);
                 else {
...
this._init = function () {
    ...
    b = g.height / 2 + 10;
    if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) g = s_oSpriteLibrary.getSprite('audio_icon'),
...
this.viewScoreBonus = function (a, b) {
    1 === b ? (r = new createjs.Text('BONUS x ' +
    a, ' 25px ' + TEXT, '#000000'), r.x = t, r.y = 50, r.textAlign = 'left', r.textBaseline = 'alphabetic', r.lineWidth = 650, r.outline = 3, s_oStage.addChild(r), q = new createjs.Text('BONUS x ' + a, ' 25px ' + TEXT, '#ffffff'), q.x = t, q.y = 50, q.textAlign = 'left', q.textBaseline = 'alphabetic', q.lineWidth = 650, s_oStage.addChild(q))  : (r.text = 'BONUS x ' + a, q.text = 'BONUS x ' + a)
};
...
this.viewKickLeft = function (a) {
    ...
    v.y = 0;
    l.addChild(v);
    for (var d = 0; d < a; d++, c += 26) b = createBitmap(s_oSpriteLibrary.getSprite('ball_kick_left')),
    b.x = x + c,
    b.y = 0,
    l.addChild(b)
};
this.unload = function () {
    if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) h.unload(),
    h = null;
    e.unload();
    s_oInterface = null
};
...
this._init = function (a, d) {
    ...
    _fbSprite = new CGfxButton(0.9 * CANVAS_WIDTH, CANVAS_HEIGHT - 130, r);
    _fbSprite.addEventListener(ON_MOUSE_UP, FacebookShare, this);
    r = s_oSpriteLibrary.getSprite('twitter');
    _twSprite = new CGfxButton(0.9 * CANVAS_WIDTH, CANVAS_HEIGHT - 220, r);
...
this.showIdle = function (f, k, h) {
    d = {
        images: [
            s_oSpriteLibrary.getSprite(h + '_idle')
        ],
        framerate: 10,
        frames: {
            width: PLAYER_WIDTH,
...
this.showIdle2 = function (f, k, h) {
    d = {
        images: [
            s_oSpriteLibrary.getSprite(h + '_idle2')
        ],
        framerate: 10,
        frames: {
            width: PLAYER_WIDTH,
...
this.showShot = function (f, k, h) {
    d = {
        images: [
            s_oSpriteLibrary.getSprite(h + '_shot')
        ],
        framerate: 15,
        frames: {
            width: PLAYER_WIDTH,
...
this.showIdle = function (f, k) {
    d = {
        images: [
            s_oSpriteLibrary.getSprite('goalkeeper_idle')
        ],
        framerate: 15,
        frames: {
...
this.showAction = function (f, k, h, e, g, l) {
    d = {
        images: [
            s_oSpriteLibrary.getSprite('goalkeeper_' +
            h)
        ],
        framerate: 15,
        frames: {
...
this._init = function (a, b) {
    ...
    for (var c = 0; c < NUM_LEVEL; c++) g[c] = Array(NUM_KICK),
    l[c] = Array(NUM_KICK),
    m[c] = Array(NUM_KICK);
    k.push({
        goalToScore: 1,
...
this.getPlayerPosIndex = function (a, b) {
    if (l[a][b] === e[0]) return 0;
    if (l[a][b] === e[1]) return 1;
    if (l[a][b] === e[2]) return 2
};
...
this.viewNextLevelPanel = function () {
    ...
    r = new createjs.Text('CONGRATULATIONS!! ', ' 60px ' + TEXT, '#000000');
    ...
    r.outline = 3;
    b.addChild(r);
    p = new createjs.Text('CONGRATULATIONS!! ', ' 60px ' + TEXT, '#ffe51f');
    ...
    v.lineWidth = 650;
    b.addChild(v);
    for (e = 0; e < d; e++, a += 26) z.push(createBitmap(s_oSpriteLibrary.getSprite('ball_kick_left'))),
...
this._updateBall = function (a, b) {
    ...
    e.rotation += 5;
    40 < c && (c = 0, f = !1, k = !0, s_oGame.showMessage(!1));
    if (!k) {
        var d;
        ...
        e.x = d.x;
        e.y = d.y;
        !0 === b && 0.7 >= e.scaleX && s_oGame.goalKeeperBounce();
        0 < a && 0.75 >= e.scaleX && s_oGame.controlWall();
        0.4 <= e.scaleX && (e.scaleX -= 0.03, e.scaleY -= 0.03)
    }
};
...
this.showIdle = function (a) {
    var b = {
        images: [
            s_oSpriteLibrary.getSprite('wall_idle')
        ],
        framerate: 10,
        frames: {
            width: WALL_WIDTH,
...
this.showJump = function (a) {
    ...
    var b = {
        images: [
            s_oSpriteLibrary.getSprite('wall_jump')
        ],
        framerate: 15,
        frames: {
            width: WALL_WIDTH,
...
this._init = function () {
    for (var d = 0; d < NUM_CROWD; d++) a.push(createBitmap(s_oSpriteLibrary.getSprite('supporters_' + d))),
    a[d].x = 0,
    a[d].y = 90,
    a[d].visible = !1,
...
this.init = function (a, d) {
    ...
    } else g = s_oSpriteLibrary.getSprite('right_bar'),
    g = createBitmap(g),
    q.addChild(g),
...
this.getPositionBallEnd = function () {
    return e ?
    Math.floor(p.y / l)  : Math.floor(p.x / l)
};
...
userName,
auth_token,
leaderboardUrl = 'http://clients.chopup.me:8081/',
siteUrl = 'http://etisalat.com.ng/fcbarca/',
_score;
...
a.getElementById(b) || (a = a.createElement(d), a.id = b, a.src = '//connect.facebook.net/en_US/sdk.js', c.parentNode.insertBefore(a, c))
...
FB.getLoginStatus(function (a) {
    'connected' === a.status ? (auth_token = a.authResponse.accessToken, FB.api('/me?fields=name,email', function (a) {
        userId = a.email;
        userName = a.name;
...
if ('null' !== userId) return console.log(' userId is ' + userId),
!0;
...
console.log(a);
'connected' === a.status ? (auth_token = a.authResponse.accessToken, handleFBResponse(), console.log('post score and show leaderboard here'))  : FB.login(function (a) {
    'connected' === a.status ? (auth_token = a.authResponse.accessToken, handleFBResponse(), console.log('post score and show leaderboard here'))  : console.log('You need to login to facebook first')
}, {
...
window.open(a, 'twitter', 'status=1,width=575,height=400,top=' +
b + ',left=' + d);
console.log('url: ' + a)
};
